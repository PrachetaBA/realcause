...
class conVAE(pl.LightningModule):
    ...
    def __init__(
        ...
        for layer in range(len(self.hidden_dim)):
            self.encoder_module.append(
                nn.Sequential(nn.Linear(self.encoder_dims[layer], self.encoder_dims[layer + 1]),
                              nn.LeakyReLU()))
        self.encoder = nn.Sequential(*self.encoder_module)

        # embedding layers
        self.en_a = nn.Sequential(nn.Linear(self.hidden_dim[-1], self.latent_dim), nn.LeakyReLU())
        self.en_b = nn.Sequential(nn.Linear(self.hidden_dim[-1], self.latent_dim), nn.LeakyReLU())
    ...
    def loss_fn(self, yhat, y, a, b, **kwargs):
        if self.potential_outcome:
        ...
        # KL divergence
        kld = torch.sum(-torch.log(a - b))    #-0.5 * torch.sum(1 + logvar - mu ** 2 - logvar.exp())
