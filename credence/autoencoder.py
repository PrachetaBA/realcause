"""Script that defines the autoencoder model to be used for generating causal inference datasets.

This script updates the script defined in the following repository:
https://github.com/harsh-parikh/credence-to-causal-estimation/blob/main/credence-v2/src/autoencoder.py
and uses a later version of lightning.

We also add additional features such as hyperparameter tuning
...
# Define the logger and the path to store logs
tb_logger = TensorBoardLogger('logs/tensorboard_logs', name='conVAE_autoencoder')


# Define the autoencoder model
class conVAE(pl.LightningModule):
    """Class that defines the autoencoder model to be used for generating causal inference datasets.

    This class trains a generator which can generate Y | X data samples.
    It takes in as input the list of column names of Y and X.
    """

    def __init__(
            self,
            df,    # Dataframe containing the source data
            Ynames,    # List of column names of Y
            Xnames=[],    # List of column names of X (which also includes the treatment variable)
            cat_cols=[],    # List of column names of categorical variables
            var_bounds={},    # Variable lower and upper bounds
            config={},    # Configuration dictionary to specify hyperparameters of the VAE
            ...
            treatment_effect_fn=lambda x:
        0,    # treatment effect function defined by the user (default is null)
            selection_bias_fn=lambda x, t:
        0,    # selection bias function defined by the user (default is null)
            effect_rigidity=0,    # strength of treatment effect constraint (default is 0, changed from original)
            bias_rigidity=0,    # strength of selection bias constraint (default is 0, changed from original)
            kld_rigidity=0,    # strength of KL divergence loss (default is 0, changed from original)
    ):
        super().__init__()
        ...
        # Get indices of the categorical variable in Y
        ...
        self.var_bounds = {Ynames.index(k): v for k, v in var_bounds.items() if k in Ynames}

        # if generating potential outcomes then add constraints for user defined treatment effects
        # and selection bias function
        if self.potential_outcome:
            self.T_col = [i for i in range(self.con_dim) if Xnames[i] in treatment_cols]
            self.X_col = [i for i in range(self.con_dim) if Xnames[i] not in treatment_cols]
        ...
        self.train_data, self.val_data = random_split(
            self.data,
            [ int(df.shape[0] * training_split),
              (df.shape[0]) - int((df.shape[0] * training_split)) ],
        )
        ...
        # embedding layers, normal distribution
    ...
    def forward(self, y):
        """Run the encoder for the input data."""
    ...
    def loss_fn(self, yhat, y, mu, logvar, **kwargs):
        """Define the loss function for the generative model."""
        if self.potential_outcome:
            ...
            # reconstruction loss
            recons = F.mse_loss(yhat_obs, y)    # RMSE between generated and observed Y

            # treatment effect constraint
            constraint_effect = torch.sum(torch.square(yhat[:, 1] - yhat[:, 0] - self.f(X)))

            # selection bias constraint
            constraint_bias = torch.sum(
                torch.square(T * (yhat[:, 0] - yhat_prime[:, 0]) + (1 - T) *
                             (yhat_prime[:, 1] - yhat[:, 1]) - self.g(X, T)))

            # adding to the loss calculation
            recons = (recons + self.alpha * constraint_effect + self.beta * constraint_bias)
        else:
            # reconstruction loss
            recons = F.mse_loss(yhat, y)

        # Enforcing a distribution on the latent space to a uniform distribution
        # KL divergence
        kld = -0.5 * torch.sum(1 + logvar - mu**2 - logvar.exp())
        recons = recons + self.kld_rigidity * kld

        # variable bounds; strongly setting constraint to enforce variable bounds
        # Probably works well only with the uniform latent space
        var_bounds_constraint = torch.tensor(0).float().to(self.device)
        for k, v in self.var_bounds.items():
            if 'lower' in v:
                lower_lim = v['lower']
                var_bounds_constraint += torch.max((yhat[:, k] < lower_lim).float()).to(self.device)
                if self.potential_outcome:
                    var_bounds_constraint += torch.max((yhat[:, k + self.in_dim]
                                                        < lower_lim).float()).to(self.device)

            if 'upper' in v:
                upper_lim = v['upper']
                var_bounds_constraint += torch.max((yhat[:, k] > upper_lim).float()).to(self.device)
                if self.potential_outcome:
                    var_bounds_constraint += torch.max((yhat[:, k + self.in_dim]
                                                        > upper_lim).float()).to(self.device)

        recons = (recons + torch.tensor(float('1e20')).to(self.device) * var_bounds_constraint)
        return recons

    def configure_optimizers(self):
        """Function to define the optimizer for the model."""
        # initializing optimizer
        optimizer = torch.optim.Adam(self.parameters(), lr=self.lr)
        return optimizer

    def training_step(self, train_batch, batch_idx):
        """Function to define the training step for the model."""
    ...
    def validation_step(self, val_batch, batch_idx):
        """Function to define the validation step for the model."""
    ...
    def fit_model(self, gpus=1, precision=16, limit_train_batches=0.5):
        """Function to call the lightning module trainer."""
