...
import numpy as np
import optax
from flowjax.bijections import (Affine, Concatenate, Invert, RationalQuadraticSpline, Stack, Tanh)
...
def train_copula_flow(
    ...
    copula_flow = Transformed(copula_flow,
                              Invert(Affine(loc=-jnp.ones(nvars),
                                            scale=jnp.ones(nvars) * 2)))    # Unbounded support
...
def train_frugal_flow_location_translation(
    ...
    list_bijections_affine = [Identity(
        (1,))] + [Invert(Affine(loc=-jnp.ones(nvars), scale=jnp.ones(nvars) * 2))]
    ...
    list_bijections_loccond = [LocCond(ate=causal_model_args['ate'])] + [Identity(())] * nvars
...
def train_frugal_flow_flexible_discrete(
    ...
    key, subkey = jr.split(key)
    outcome, _ = univariate_discrete_cdf(key=subkey,
                                         z_discr=y.flatten(),
                                         max_unique_z_discr_size=len(jnp.unique(y)))
    ...
    list_bijections_affine_output = [Invert(Affine(loc=-jnp.ones(1), scale=jnp.ones(1) * 2))
                                    ] + [Identity((nvars,))]
...
def train_frugal_flow(
    ...
    if u_z_hetero is not None:
        assert condition.shape[1] == (
            u_z_hetero.shape[1] +
            1), 'Both z_hetero and treatment must be included in the condition.'
...
def get_independent_quantiles(
    ...
    def rankdata(z_disc):
        ...
        for d in range(z_disc.shape[1]):
            ...
            rank_mapping = {
                k: v for k, v in zip(np.array(unique_z_disc_d), np.arange(len(unique_z_disc_d)))
            }
...
def univariate_marginal_cdf(
    ...
    if z_cont.ndim == 1:
        ...
        raise ValueError('Univariate input with shape (n_samples,) or (n_samples,1) is required')
