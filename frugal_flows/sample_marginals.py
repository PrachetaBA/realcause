...
def from_quantiles_to_marginal_cont(
    ...
    if isinstance(flow, list):
        ...
        for i, flow_i in enumerate(flow):
            ...
            uni_minus1_plus1 = jax.vmap(flow_i.bijection.bijections[0].tree.transform)(uni_standard)

            corruni_minus1_plus1 = jax.vmap(
                flow_i.bijection.bijections[1].transform)(uni_minus1_plus1)
            marginal_sample = jax.vmap(
                flow_i.bijection.bijections[2].transform)(corruni_minus1_plus1)
        ...
        uni_minus1_plus1 = jax.vmap(flow.bijection.bijections[0].tree.transform)(uni_standard)

        corruni_minus1_plus1 = jax.vmap(flow.bijection.bijections[1].transform)(uni_minus1_plus1)
        marginal_sample = jax.vmap(flow.bijection.bijections[2].transform)(corruni_minus1_plus1)
        return marginal_sample
    else:
        raise ValueError('Pass valid flow object or list of valid univariate flow objects')
