...
from frugal_flows.bijections import (MaskedAutoregressiveFirstUniform,
...
def _add_default_permute_but_first(bijection: AbstractBijection, dim: int, key: Array):
    ...
    perm = Permute(
        jnp.hstack([jnp.expand_dims(0, axis=-1), jr.permutation(key, jnp.arange(1, dim))]))
...
def masked_autoregressive_flow_heterogeneous(
    ...
    *,
    ...
    def make_layer(key):    # masked autoregressive layer + permutation
        ...
        # bijection = Concatenate(list_bijections)
        bijection = MAF_bijection
        return _add_default_permute_but_specific_idx(bijection, dim, perm_key, causal_effect_idx)
...
def _add_default_permute_but_specific_idx(bijection: AbstractBijection,
                                          dim: int,
                                          key: Array,
                                          idx: int):
    if (dim == 1) or (dim == 2):
