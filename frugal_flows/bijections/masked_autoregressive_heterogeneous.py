...
class MaskedAutoregressiveHeterogeneous(AbstractBijection):
    ...
    def __init__(
        ...
        *,
        ...
        if transformer.shape != () or transformer.cond_shape is not None:
            raise ValueError('Only unconditional transformers with shape () are supported.',)
        ...
        self.masked_autoregressive_mlp = (masked_autoregressive_mlp_stopped_heterogeneous(
            in_ranks,
            hidden_ranks,
            out_ranks,
            depth=nn_depth,
    ...
    def transform(self, x, condition=None):
        ...
        transformer_params = self.masked_autoregressive_mlp(nn_input,
                                                            stop_grad_until=self.stop_grad_until)
    ...
    def _flat_params_to_transformer(self, params: Array):
        ...
        if self.identity_idx > 0:
            ...
            transformer_b = eqx.filter_vmap(self.transformer_constructor)(transformer_params_b)
        ...
        if self.identity_idx < dim - 1:
            ...
            transformer_a = eqx.filter_vmap(self.transformer_constructor)(transformer_params_a)
...
class StoppedMLPHeterogeneous(eqx.nn.MLP):
    ...
    def __call__(self, x, identity_idx=None, stop_grad_until=None):
        ...
        if stop_grad_until is not None:
            params_stopped = transformer_params[(stop_grad_until * identity_idx +
                                                 1):((stop_grad_until * (identity_idx + 1)) + 1)]
            params_stopped = jax.lax.stop_gradient(params_stopped)
            transformer_params = transformer_params.at[(stop_grad_until * identity_idx +
                                                        1):((stop_grad_until * (identity_idx + 1)) +
                                                            1)].set(params_stopped)
...
def masked_autoregressive_mlp_stopped_heterogeneous(
    ...
    **kwargs,
    ...
    for i, linear in enumerate(mlp.layers):
        ...
        masked_linear = eqx.tree_at(lambda linear: linear.weight,
                                    linear,
                                    Where(mask, linear.weight, 0))
