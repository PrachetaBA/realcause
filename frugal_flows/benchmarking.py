...
from frugal_flows.sample_marginals import (from_quantiles_to_marginal_cont,
...
class FrugalFlowModel:
    ...
    def _bivariate_gaussian_copula(self, key, N, rho):
        corr_matrix = jnp.array([[1., rho], [rho, 1.]])
        mean = jnp.array([0, 0])
    ...
    def train_benchmark_model(self,
        ...
        self.train_frugal_flow(training_seeds[1],
                               frugal_hyperparam_dict,
                               causal_model,
                               causal_model_args)
    ...
    def train_marginal_cdfs(self, key, hyperparam_dict):
        self.res = get_independent_quantiles(key=key,
                                             z_cont=self.Z_cont,
                                             z_discr=self.Z_disc,
    ...
    def train_frugal_flow(self, key, hyperparam_dict, causal_model, causal_model_args):
        ...
        self.frugal_flow, losses = train_frugal_flow(key=key,
                                                     y=self.Y,
                                                     u_z=uz_full_samples,
                                                     condition=self.X,
    ...
    def train_propensity_flow(self, key, hyperparam_dict):
        ...
        self.prop_flow, _ = train_quantile_propensity_score(key=key,
                                                            x=self.X.astype(int),
                                                            condition=condition,
                                                            **hyperparam_dict)
    ...
    def generate_samples(self,
        ...
        uz_samples = self.vmap_frugal_flow(x=frugal_baselines, condition=jnp.zeros(u_yx.shape))[:,
                                                                                                1:]
        ...
        if outcome_causal_model == 'location_translation':
            Y_samples = sample_outcome(frugal_flow=self.frugal_flow,
                                       key=subkeys[4],
                                       n_samples=sampling_size,
                                       causal_model=outcome_causal_model,
                                       causal_condition=X_samples,
                                       u_yx=u_yx.flatten(),
                                       **outcome_causal_args)[:, None]
        else:
            Y_samples = sample_outcome(key=subkeys[4],
                                       n_samples=sampling_size,
                                       causal_model=outcome_causal_model,
                                       causal_condition=X_samples,
                                       u_yx=u_yx.flatten(),
        ...
        sim_data_df = pd.DataFrame(
            sim_data, columns=['Y', 'X', *[f'Z_{i+1}' for i in range(full_Z_samples.shape[1])]])
