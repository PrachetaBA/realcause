...
from flowjax.bijections import Affine, Invert, Tanh
...
def sample_outcome(
    ...
    **treatment_kwargs: dict,
    ...
    if ((u_yx is not None) & (frugal_flow is not None) & (causal_model != 'location_translation')):
        raise ValueError(
    ...
    if u_yx is not None:
        ...
        spline_n_params = int(frugal_flow.bijection.bijections[1].bijection.bijection.bijections[0].
                              masked_autoregressive_mlp.layers[-1].out_features / maf_dim)
        assert (frugal_flow.bijection.bijections[1].bijection.bijection.bijections[0].
                transformer_constructor(jnp.ones((spline_n_params))).interval == 1)
        try:
            assert isinstance(frugal_flow.bijection.bijections[2].tree.bijections[0], Identity)
        except Exception:
            assert (isinstance(frugal_flow.bijection.bijections[2].tree, Invert)) & (isinstance(
                frugal_flow.bijection.bijections[2].tree.bijection, Affine))

        # obtain u_y samples from flow
        uni_standard = jr.uniform(key, shape=(n_samples, flow_dim))
        uni_minus1_plus1 = jax.vmap(
            frugal_flow.bijection.bijections[0].tree.transform)(uni_standard)

        corruni_minus1_plus1 = jax.vmap(frugal_flow.bijection.bijections[1].transform)(
            uni_minus1_plus1, flow_fake_condition)
        corruni = jax.vmap(frugal_flow.bijection.bijections[2].tree.transform)(corruni_minus1_plus1,
                                                                               flow_fake_condition)
...
def logistic_outcome(u_y: ArrayLike, ate: float, causal_condition: ArrayLike, const: float):
    """
...
def causal_cdf_outcome(
    ...
    **treatment_kwargs: dict,
    ...
    causal_cdf_init_params = [
        i for i in inspect.signature(causal_cdf.__init__).parameters.keys()
...
def location_translation_outcome(
    ...
    **treatment_kwargs: dict,
    ...
    causal_minus1_plus1 = jax.vmap(frugal_flow.bijection.bijections[3].bijections[0].transform)(
        u_y[:, None], flow_condition)
    causal_reals = jax.vmap(frugal_flow.bijection.bijections[4].bijections[0].transform)(
