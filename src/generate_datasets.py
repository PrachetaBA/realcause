...
def generate_datasets(gen_datasets_folder, best_model_path, **kwargs):
    ...
    d = get_apo_data(identifier='acic', data_format='pandas',
                     weight=weight, intercept=intercept)
    ...
    dfs = []
    print(f'Generating {N_SAMPLE_SEEDS} datasets with {N_AGG_SEEDS} seeds for each samples')
